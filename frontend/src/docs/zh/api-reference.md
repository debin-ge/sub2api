...
本页列出 Sub2API 常用兼容端点。实际支持范围取决于当前部署的路由、分组、上游账号、模型映射、渠道价格和管理员配置。
...
部分兼容客户端可能使用 `api-key`、`x-api-key` 或 SDK 内置字段传递密钥。除非管理员特别说明，推荐优先使用 `Authorization: Bearer $SUB2API_KEY`。
...
成功响应通常遵循对应兼容接口的结构。错误响应可能来自 Sub2API 本身，也可能来自上游平台。排查时优先确认：
