...
> [!IMPORTANT]
> **Important**: The features available in a specific deployment depend on its admin configuration, including the user's group, upstream account status, model mappings, channel pricing, quota policy, and enabled endpoints. **Do not assume every {{SITE_NAME}} deployment supports every model or API.**
...
| Step | Recommendation |
| :--- | :--- |
| **1. Verify the key** | Call `${BASE_URL}v1/models` first to confirm authentication and model visibility. |
| **2. Choose an API** | OpenAI SDK clients usually use `/v1/chat/completions` or `/v1/responses`; Claude-style clients use `/v1/messages`; Gemini native clients use `/v1beta` paths. |
| **3. Pin model names** | Use model names returned by `/v1/models`, or mapping names explicitly provided by an admin. |
| **4. Enable streaming** | Use `stream: true` or SSE endpoints only after confirming your client supports streaming responses. |
| **5. Troubleshoot failures** | For 401, 403, 404, 429, or upstream 5xx responses, first check whether the key, group, endpoint, and model match the current deployment. |
...
| Goal | Read |
| :--- | :--- |
| **Make your first API call** | [Quick Start](quickstart) ➔ [API Keys and Accounts](api-keys) |
| **Configure a concrete client** | [Client Integration](clients) ➔ [Copy-Ready Configuration Snippets](configuration-snippets) |
| **Choose the right API endpoint** | [Endpoint Selection Guide](endpoint-selection) ➔ [API Reference](api-reference) |
| **Choose the right model** | Read the selection method in [Models and Platforms](models). |
| **Understand balance and cost** | [Billing and Usage](billing-usage) |
| **Fix request failures or unexpected behavior** | [Troubleshooting](errors) ➔ [FAQ](faq) |
| **Prepare for production** | Read the launch checklist in [Best Practices](best-practices). |
