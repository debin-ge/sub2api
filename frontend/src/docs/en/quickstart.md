...
This page shows how to send your first request through {{SITE_NAME}}.

## 1. Confirm the Base URL

The Base URL is the address of your {{SITE_NAME}} deployment, already filled into the examples:

```text
{{BASE_URL}}
```

If your deployment is behind a reverse proxy path, use the full address provided by your admin.

## 2. Set the API Key

{{SITE_NAME}} API Keys are passed through HTTP Bearer authentication:

```http
Authorization: Bearer ${API_KEY}
```

The command-line examples use `${API_KEY}`. Store your key in an environment variable first (replace the value with your real key):
...
| API Key | Use `Authorization: Bearer ${API_KEY}`. |
