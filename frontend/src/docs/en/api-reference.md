...
> [!NOTE]
> Some compatible clients may send keys through `api-key`, `x-api-key`, or SDK-specific fields. Unless your admin says otherwise, prefer `Authorization: Bearer $YOUR_KEY` in any integration scenario.
...
| Method | Path | Purpose | Notes |
| :--- | :--- | :--- | :--- |
| **GET** | `/v1beta/models` | List Gemini models | The response only includes models accessible under current settings. |
...
> [!IMPORTANT]
> **Successful responses** usually follow the corresponding compatible API structure. **Errors** may come from {{SITE_NAME}} itself or from an upstream platform.

When troubleshooting, check in order:
1. Whether the API Key is correct and has a `Bearer ` prefix.
