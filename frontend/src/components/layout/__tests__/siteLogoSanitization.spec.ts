...
const siteHeaderSource = readFileSync(resolve(dir, '../../common/SiteHeader.vue'), 'utf8')
...
describe('site_logo sanitization', () => {
  ...
  it('SiteHeader applies sanitizeUrl to siteLogo', () => {
    expect(siteHeaderSource).toContain('sanitizeUrl(')
    expect(siteHeaderSource).toContain('appStore.cachedPublicSettings?.site_logo || appStore.siteLogo')
  })
  ...
  it('all three pass allowRelative and allowDataUrl options', () => {
    for (const src of [sidebarSource, siteHeaderSource, keyUsageViewSource]) {
