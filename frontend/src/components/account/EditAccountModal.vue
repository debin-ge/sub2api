    ...
    >
      ...
      <div v-if="headerOverrideCapable" class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div v-if="headerOverrideEnabled" class="space-y-3">
          ...
          <HeaderOverrideEditor
            ...
            :template-platform="account.platform"
            add-button-test-id="edit-header-override-add-row"
      ...
      >
        ...
        >
          ...
          <button
            type="button"
            data-testid="edit-openai-codex-allow-claude-code-toggle"
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (newAccount.platform === 'glm' || newAccount.platform === 'opencode_go' || isCNProviderPlatform(newAccount.platform)) {
    ...
    // Load model mappings for OAuth/Setup-Token accounts (Antigravity 使用独立的专属状态)
    if (newAccount.platform !== 'antigravity' && newAccount.credentials) {
  ...
  try {
    ...
    if (props.account.type === 'apikey') {
      ...
      if (props.account.platform === 'minimax') {
        ...
        delete newCredentials.temp_unschedulable_enabled
        delete newCredentials.temp_unschedulable_rules
      } else if (props.account.platform === 'opencode_go') {
        delete newCredentials.auth_scheme
        delete newCredentials.base_url_anthropic
        delete newCredentials.base_url_openai
      ...
      if (isCNApiKeyAccount.value) {
        ...
        if (editApiProtocol.value === 'adaptive') {
          ...
          if (props.account.platform === 'minimax') {
            protocolBaseUrls.chat_completions = editMiniMaxOpenAIBaseUrl.value.trim() || defaults.chat_completions
            protocolBaseUrls.anthropic = editMiniMaxAnthropicBaseUrl.value.trim() || defaults.anthropic
            protocolBaseUrls.responses = editMiniMaxOpenAIBaseUrl.value.trim() || defaults.responses
          } else {
            for (const item of editAdaptiveProtocolOptions.value) {
          ...
          if (props.account.platform === 'minimax') {
            newCredentials.base_url = editApiProtocol.value === 'anthropic'
              ? newCredentials.base_url_anthropic
              : newCredentials.base_url_openai
          }
        }
    ...
    // OAuth/Setup-Token: persist model mapping to credentials (Antigravity 使用独立的专属逻辑).
    if ((props.account.type === 'oauth' || props.account.type === 'setup-token') && props.account.platform !== 'antigravity') {
