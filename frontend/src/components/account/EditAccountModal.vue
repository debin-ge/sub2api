...
const isCNApiKeyAccount = computed(
  () =>
    ...
    (props.account.platform === 'glm' || isCNProviderPlatform(props.account.platform))
)
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    if (newAccount.platform === 'glm' || isCNProviderPlatform(newAccount.platform)) {
      ...
      const adaptiveDefaults = defaultCNAdaptiveBaseUrls(cnPresetPlatform.value, editAccountMode.value)
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if ((props.account.platform === 'openai' || props.account.platform === 'grok') && props.account.type === 'oauth') {
      ...
      const newCredentials: Record<string, unknown> = { ...currentCredentials }
      const shouldApplyModelMapping =
        props.account.platform === 'grok' || !openaiPassthroughEnabled.value
