...
const apiKeyHint = computed(() => {
  ...
  if (form.platform === 'opencode_go') return t('admin.accounts.opencode.apiKeyHint')
...
function selectCNPlatform(platform: CnProviderPlatform) {
  ...
  if (platform === 'minimax') {
    minimaxAnthropicBaseUrl.value = adaptiveBaseUrls.value.anthropic
    minimaxOpenAIBaseUrl.value = adaptiveBaseUrls.value.chat_completions
  }
}
...
const syncPreviewCredentials = computed(() => {
  ...
  switch (form.platform) {
	...
	if (isMultiProtocolPlatform.value && form.platform !== 'minimax' && apiProtocol.value === 'adaptive') {
		baseUrl = adaptiveBaseUrls.value.chat_completions.trim() || apiKeyBaseUrl.value.trim() || undefined
	}
...
const handleSubmit = async () => {
  ...
  if (isCNProviderPlatform(form.platform) || form.platform === 'opencode_go') {
    ...
    if (apiProtocol.value === 'adaptive') {
      ...
      if (form.platform === 'minimax') {
        protocolBaseUrls.chat_completions = minimaxOpenAIBaseUrl.value.trim() || defaults.chat_completions
        protocolBaseUrls.anthropic = minimaxAnthropicBaseUrl.value.trim() || defaults.anthropic
        protocolBaseUrls.responses = minimaxOpenAIBaseUrl.value.trim() || defaults.responses
      } else {
        for (const item of cnAdaptiveProtocolOptions.value) {
    ...
    if (form.platform === 'minimax') {
      credentials.auth_scheme = 'bearer'
      credentials.base_url_anthropic = minimaxAnthropicBaseUrl.value.trim() || MINIMAX_ANTHROPIC_BASE_URL
      credentials.base_url_openai = minimaxOpenAIBaseUrl.value.trim() || MINIMAX_OPENAI_BASE_URL
      if (apiProtocol.value !== 'adaptive') {
        credentials.base_url = apiProtocol.value === 'anthropic'
          ? credentials.base_url_anthropic
          : credentials.base_url_openai
      }
    }
    if (form.platform === 'opencode_go') {
