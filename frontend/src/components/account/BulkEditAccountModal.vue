        ...
        <div v-if="enableHeaderOverride" id="bulk-edit-header-override-body" class="mt-3 space-y-3">
          ...
          <div v-if="headerOverrideEnabled" class="space-y-3">
            ...
            <HeaderOverrideEditor
              ...
              add-button-id="bulk-edit-header-override-add-row"
...
const targetMayIncludeFixedEndpointGateway = computed(() => {
  if (targetSelectedPlatforms.value.some(p => FIXED_ENDPOINT_GATEWAY_PLATFORMS.includes(p))) {
    return true
  }
  if (targetMode.value !== 'filtered') {
    return false
  }
  return filteredPlatform.value === '' || FIXED_ENDPOINT_GATEWAY_PLATFORMS.includes(filteredPlatform.value)
})

const targetIsFixedEndpointGatewayAPIKeyOnly = computed(() => (
  capabilityPlatforms.value.length === 1 &&
  FIXED_ENDPOINT_GATEWAY_PLATFORMS.includes(capabilityPlatforms.value[0]) &&
...
const allHeaderOverrideCapable = computed(() => {
  return (
    capabilityPlatforms.value.length > 0 &&
    capabilityTypes.value.length > 0 &&
    capabilityPlatforms.value.every(p =>
      capabilityTypes.value.every(ty => isHeaderOverrideCapable(p, ty))
    )
  )
})
...
const headerOverrideTemplatePlatform = computed(() => {
  return capabilityPlatforms.value.length === 1 ? capabilityPlatforms.value[0] : null
})
...
const buildUpdatePayload = (): Record<string, unknown> | null => {
  ...
  if (allBillingProbeCapable.value && enableUpstreamBillingAutoProbe.value) {
  ...
  if (
    ...
    const extra = ensureExtra()
    extra.codex_cli_only_allowed_clients = codexCLIOnlyAllowClaudeCodeEnabled.value
      ? ['claude_code']
      : []
  }
