<template>
  ...
  <div class="flex flex-wrap gap-2">
    <button
      v-if="templatePlatform && getHeaderOverrideTemplate(templatePlatform).length > 0"
      type="button"
...
const fillTemplate = () => {
  if (!props.templatePlatform) return
  const existing = new Set(props.rows.map((row) => row.name.trim().toLowerCase()).filter(Boolean))
  const rows = props.rows.filter((row) => row.name.trim() || row.value.trim())
  for (const row of getHeaderOverrideTemplate(props.templatePlatform)) {
