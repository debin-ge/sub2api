      ...
      >
        <div class="py-1">
          <template v-if="account">
            ...
            <button v-if="isMiniMaxApiKey" @click="$emit('sync-minimax-remains', account); $emit('close')" class="flex w-full items-center gap-2 px-4 py-2 text-sm text-teal-600 hover:bg-gray-100 dark:hover:bg-dark-700">
              <Icon name="refresh" size="sm" />
...
const hasQuotaLimit = computed(() => {
  return !isShadow.value && !isFixedEndpointGatewayPlatform.value &&
