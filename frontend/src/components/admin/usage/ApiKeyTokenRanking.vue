<template>
  ...
  <div>
    ...
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-dark-700/50 sm:px-6">
      <p class="text-xs text-gray-400 dark:text-gray-500">{{ t(`${i18nPrefix}.subtitle`) }}</p>
      ...
      <table class="w-full min-w-max divide-y divide-gray-200 dark:divide-dark-700">
        <thead class="bg-gray-50 dark:bg-dark-800">
          <tr>
            ...
            <th v-if="mode === 'admin'" class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-dark-400">
        ...
        <tbody class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
          ...
          <tr v-else-if="items.length === 0">
            <td :colspan="sortableColumns.length + (mode === 'admin' ? 3 : 2)" class="py-12 text-center text-sm text-gray-400">
          ...
          >
            ...
            <td class="max-w-[260px] truncate px-4 py-3 text-sm font-medium text-gray-700 dark:text-gray-200" :title="item.key_name">
              {{ item.key_name || `Key #${item.api_key_id}` }}
              <span class="ml-1 font-normal text-gray-400 dark:text-gray-500">#{{ item.api_key_id }}</span>
            </td>
            <td v-if="mode === 'admin'" class="max-w-[220px] truncate px-4 py-3 text-sm text-gray-500 dark:text-gray-400" :title="item.email">
