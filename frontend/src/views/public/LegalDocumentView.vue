...
import { renderAdminComplianceDocumentTemplate } from '@/utils/adminComplianceDocument'
...
const siteName = computed(() => settings.value?.site_name || 'ZenTok')
...
const currentDocument = computed<LoginAgreementDocument | null>(() => {
  if (isAdminComplianceDocument.value) {
    return {
      ...
      content_md: renderAdminComplianceDocumentTemplate(
        getLocale() === 'zh' ? zhAdminCompliance : enAdminCompliance,
        siteName.value
      )
    }
  }
