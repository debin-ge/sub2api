...
const handleSyncMiniMaxRemains = async (a: Account) => {
  try {
    const updated = await adminAPI.accounts.syncMiniMaxRemains(a.id)
    patchAccountInList(updated)
    enterAutoRefreshSilentWindow()
    appStore.showSuccess(t('admin.accounts.syncMiniMaxRemainsSuccess'))
  } catch (error: any) {
    console.error('Failed to sync MiniMax remains:', error)
    appStore.showError(error?.response?.data?.message || error?.message || t('admin.accounts.syncMiniMaxRemainsFailed'))
  }
}
