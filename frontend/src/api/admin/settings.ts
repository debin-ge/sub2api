...
const AUTH_SOURCE_DEFAULTS_VALUE: AuthSourceDefaultsValue = {
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    const current = authSourceDefaults[source] ?? AUTH_SOURCE_DEFAULTS_VALUE;
...
export interface SystemSettings {
  ...
  affiliate_registration_reward_amount: number;
...
export interface UpdateSettingsRequest {
  ...
  invitation_code_enabled?: boolean;
  invitation_code_required?: boolean;
  registration_rate_limit_per_ip?: number;
