...
import type { GroupCatalogEntry } from '@/types'
 ...
 */
export async function getAvailable(): Promise<GroupCatalogEntry[]> {
  const { data } = await apiClient.get<GroupCatalogEntry[]>('/groups/available')
  return data
}
