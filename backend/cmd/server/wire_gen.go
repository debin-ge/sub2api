...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	goCaptchaGenerator := repository.NewGoCaptchaGenerator(configConfig)
	goCaptchaCache := repository.NewGoCaptchaCache(redisClient)
	goCaptchaService := service.NewGoCaptchaService(settingService, goCaptchaGenerator, goCaptchaCache, configConfig)
	...
	modelPriceOverrideStore := repository.NewModelPriceOverrideRepository(client)
	modelPriceOverrideCache := repository.NewModelPriceOverrideCache(redisClient)
	pricingService, err := service.ProvidePricingService(configConfig, pricingRemoteClient, modelPriceOverrideStore, modelPriceOverrideCache)
	if err != nil {
		return nil, err
	}
	...
	groupHandler := admin.NewGroupHandlerWithConfig(adminService, dashboardService, groupCapacityService, modelCatalogService, configConfig)
	...
	byteDanceVideoProvider := service.ProvideByteDanceVideoProvider(httpUpstream, tlsFingerprintProfileService, videoCapabilityCatalog)
	videoProviderRegistry := service.ProvideVideoProviderRegistry(openAIVideoProvider, byteDanceVideoProvider)
	...
	modelPriceHandler := admin.ProvideModelPriceHandler(pricingService, modelCatalogService, billingService)
	...
	openAIGatewayHandler := handler.ProvideCompatibleGatewayHandler(openAIGatewayService, pluginManager, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator)
	...
	captchaHandler := handler.NewCaptchaHandler(goCaptchaService)
	...
	availableChannelHandler := handler.NewAvailableChannelHandler(channelService, apiKeyService, settingService, modelCatalogService, publicModelStatsProvider, billingFallbackProvider, pricingService)
