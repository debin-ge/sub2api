...
func TestResolveCLIVersion(t *testing.T) {
	cases := []struct {
		...
		{"旧的内置基线回落", "2.1.260", CLICurrentVersion},
...
func TestCLIVersionMatchesStartupConfiguration(t *testing.T) {
	want := resolveCLIVersion(os.Getenv(CLIVersionEnv))
	if got := CLIVersion(); got != want {
		t.Fatalf("CLIVersion() = %q, want %q", got, want)
	}
}
