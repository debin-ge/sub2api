...
func NewGroupHandler(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, catalog *service.ModelCatalogService) *GroupHandler {
	return NewGroupHandlerWithConfig(adminService, dashboardService, groupCapacityService, catalog, nil)
}

func NewGroupHandlerWithConfig(adminService service.AdminService, dashboardService *service.DashboardService, groupCapacityService *service.GroupCapacityService, catalog *service.ModelCatalogService, cfg *config.Config) *GroupHandler {
...
type CreateGroupRequest struct {
	...
	Description               string                        `json:"description"`
	Platform                  string                        `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok minimax kimi zhipu deepseek windsurf opencode_go bytedance composite"`
...
type UpdateGroupRequest struct {
	Name                      string                         `json:"name"`
	Description               *string                        `json:"description"`
	Platform                  string                         `json:"platform" binding:"omitempty,oneof=anthropic openai gemini antigravity grok minimax glm kimi zhipu deepseek windsurf opencode_go bytedance composite"`
	RateMultiplier            *float64                       `json:"rate_multiplier"`
	IsExclusive               *bool                          `json:"is_exclusive"`
	VIPOnly                   *bool                          `json:"vip_only"`
...
type CompositeRouteRequest struct {
	...
	TargetPlatform string `json:"target_platform" binding:"required,oneof=anthropic openai gemini antigravity grok kimi zhipu deepseek minimax windsurf opencode_go bytedance"`
...
// GetModelsListCandidates is retained for compatibility with older admin clients.
