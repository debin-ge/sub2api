...
func TestAccountHandlerGetAvailableModels_OpenAIAPIKeyDefaultsToConcreteGPT56Sol(t *testing.T) {
	...
	router := setupAvailableModelsRouter(svc, &adminModelCatalogStub{
		accountModels: map[int64][]string{
			46: service.DefaultModelCatalogIDs(service.PlatformOpenAI),
		},
	})
...
func TestAccountHandlerGetAvailableModels_GeminiGoogleOneUsesConservativeCatalog(t *testing.T) {
	...
	router := setupAvailableModelsRouter(svc, &adminModelCatalogStub{accountModels: map[int64][]string{
		45: {"gemini-2.0-flash", "gemini-2.5-flash", "gemini-2.5-pro"},
	}})
