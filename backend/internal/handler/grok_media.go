	...
	for {
		...
		if err != nil {
			...
			if len(failedAccountIDs) == 0 {
				cls := classifyNoAccountErrorFromGin(c, h.gatewayService, apiKey, err, requestModel, routingModel, platform)
		...
		if endpoint == service.SeedanceEndpointStatus {
			if billResult := prepareSeedanceCompletionBilling(requestCtx, h, apiKey, subject, requestID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, billResult, endpoint, billResult.Model, body, requestID)
			}
			...
			if billResult := prepareGrokVideoCompletionBilling(requestCtx, h, reqLog, apiKey, subject, taskID, result); billResult != nil {
				recordGrokMediaUsage(c, h, reqLog, apiKey, subject, subscription, account, billResult, endpoint, billResult.Model, body, taskID)
			}
