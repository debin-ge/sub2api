...
type radarHandlerCall struct {
	...
	model  string
	bucket string
	metric service.DegradationMetric
	days   int
}
...
type fakeRadarPublicService struct {
	...
	trend          *service.DegradationTrendDTO
...
func (f *fakeRadarPublicService) GetDegradationTrend(_ context.Context, model string, metric service.DegradationMetric, days int) (*service.DegradationTrendDTO, error) {
	f.record(radarHandlerCall{method: "degradation/trend", model: model, metric: metric, days: days})
...
func newRadarHandlerTestConfig(slugs ...string) *config.Config {
	return &config.Config{Radar: config.RadarConfig{ArtificialAnalysisModelSlugs: slugs}}
}
...
func TestNewRadarHandlerRejectsNilDependenciesAndUnsafeConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "empty configured slug", cfg: newRadarHandlerTestConfig(""), radarService: &fakeRadarPublicService{}},
		{name: "blank configured slug", cfg: newRadarHandlerTestConfig(" \t "), radarService: &fakeRadarPublicService{}},
		{name: "unsafe configured slug", cfg: newRadarHandlerTestConfig("../redis-secret"), radarService: &fakeRadarPublicService{}},
	}
...
func TestRadarHandlerSuccessUsesEnvelopeAndExactCacheControl(t *testing.T) {
	fake := &fakeRadarPublicService{
		...
		trend:       &service.DegradationTrendDTO{ModelSlug: "model-a", Metric: service.DegradationMetricCodingIndex, Days: 90, DataPoints: make([]service.MetricPointDTO, 0)},
	...
	tests := []struct {
		...
		{name: "degradation latest", handler: h.GetDegradationLatest, cacheControl: "public, max-age=3600"},
		{name: "degradation trend", handler: h.GetDegradationTrend, query: "?model=model-a&metric=coding_index", cacheControl: "public, max-age=3600"},
		{name: "lmarena", handler: h.GetLMArena, cacheControl: "public, max-age=3600"},
	...
	require.Contains(t, fake.snapshotCalls(), radarHandlerCall{
		method: "degradation/trend",
		model:  "model-a",
		metric: service.DegradationMetricCodingIndex,
		days:   90,
	})
	require.Contains(t, fake.snapshotCalls(), radarHandlerCall{
...
func TestRadarHandlerTrendValidatesBeforeCallingService(t *testing.T) {
	tests := []struct {
		name  string
		query string
	}{
		{name: "missing model", query: "?metric=coding_index"},
		{name: "empty model", query: "?model=&metric=coding_index"},
		{name: "missing metric", query: "?model=model-a"},
		{name: "empty metric", query: "?model=model-a&metric="},
		{name: "unsupported metric", query: "?model=model-a&metric=elo"},
		{name: "uppercase metric", query: "?model=model-a&metric=CODING_INDEX"},
		{name: "model outside allowlist", query: "?model=model-b&metric=coding_index"},
		{name: "model matching only by case", query: "?model=MODEL-A&metric=coding_index"},
		{name: "space padded model", query: "?model=%20model-a%20&metric=coding_index"},
		{name: "unicode padded model", query: "?model=%E3%80%80model-a&metric=coding_index"},
		{name: "overlong model", query: "?model=" + strings.Repeat("a", 129) + "&metric=coding_index"},
		{name: "unknown query key", query: "?model=model-a&metric=coding_index&extra=1"},
		{name: "blank query key", query: "?model=model-a&metric=coding_index&=1"},
		{name: "empty days", query: "?model=model-a&metric=coding_index&days="},
		{name: "zero days", query: "?model=model-a&metric=coding_index&days=0"},
		{name: "days above maximum", query: "?model=model-a&metric=coding_index&days=91"},
		{name: "negative days", query: "?model=model-a&metric=coding_index&days=-1"},
		{name: "non decimal days", query: "?model=model-a&metric=coding_index&days=1.0"},
		{name: "explicit plus days", query: "?model=model-a&metric=coding_index&days=%2B1"},
		{name: "space padded days", query: "?model=model-a&metric=coding_index&days=%201%20"},
		{name: "overflow days", query: "?model=model-a&metric=coding_index&days=999999999999999999999999"},
		{name: "repeated model", query: "?model=model-a&model=model-a&metric=coding_index"},
		{name: "repeated metric", query: "?model=model-a&metric=coding_index&metric=coding_index"},
		{name: "repeated days", query: "?model=model-a&metric=coding_index&days=30&days=30"},
		{name: "malformed days escape", query: "?model=model-a&metric=coding_index&days=%zz"},
		{name: "bare semicolon", query: "?model=model-a&metric=coding_index;"},
		{name: "malformed duplicate model", query: "?model=model-a&model=%zz&metric=coding_index"},
		{name: "malformed duplicate metric", query: "?model=model-a&metric=coding_index&metric=%zz"},
		{name: "malformed duplicate days", query: "?model=model-a&metric=coding_index&days=30&days=%zz"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			w := performRadarHandlerRequest(h.GetDegradationTrend, tt.query)
...
func TestRadarHandlerTrendMatchesConfigNormalizationAndCopiesAllowlist(t *testing.T) {
	fake := &fakeRadarPublicService{trend: &service.DegradationTrendDTO{DataPoints: make([]service.MetricPointDTO, 0)}}
	cfg := newRadarHandlerTestConfig(" model-a ", "model-a")
	h, err := NewRadarHandler(cfg, fake)
	require.NoError(t, err)
	cfg.Radar.ArtificialAnalysisModelSlugs[0] = "model-b"
	cfg.Radar.ArtificialAnalysisModelSlugs[1] = "model-c"

	w := performRadarHandlerRequest(h.GetDegradationTrend, "?model=model-a&metric=agentic_index&days=1")

	require.Equal(t, http.StatusOK, w.Code)
	require.Equal(t, []radarHandlerCall{{
		method: "degradation/trend",
		model:  "model-a",
		metric: service.DegradationMetricAgenticIndex,
		days:   1,
	}}, fake.snapshotCalls())
}
...
func TestRadarHandlerMapsServiceErrorsWithoutExposingDetails(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "degradation trend", setErr: func(f *fakeRadarPublicService, err error) { f.trendErr = err }, handler: func(h *RadarHandler) gin.HandlerFunc { return h.GetDegradationTrend }, query: "?model=model-a&metric=intelligence_index"},
...
func TestRadarHandlerTrendSupportsEveryMetricAndExplicitDayBoundary(t *testing.T) {
	tests := []struct {
		metric string
		days   int
	}{
		{metric: "intelligence_index", days: 1},
		{metric: "coding_index", days: 90},
		{metric: "agentic_index", days: 7},
	}
	...
	for _, tt := range tests {
		t.Run(tt.metric, func(t *testing.T) {
			...
			w := performRadarHandlerRequest(h.GetDegradationTrend, "?model=model-a&metric="+tt.metric+"&days="+strconv.Itoa(tt.days))

			require.Equal(t, http.StatusOK, w.Code)
			calls := fake.snapshotCalls()
			require.Len(t, calls, 1)
			require.Equal(t, service.DegradationMetric(tt.metric), calls[0].metric)
			require.Equal(t, tt.days, calls[0].days)
		})
	}
}
...
func TestRadarHandlerErrorBodyNeverEchoesMalformedQuery(t *testing.T) {
	...
	w := performRadarHandlerRequest(h.GetDegradationTrend, "?model=redis%3Asecret-key&metric=coding_index&days=secret-api-key")
