...
func (h *KimiGatewayHandler) Messages(c *gin.Context) {
	...
	usageCtx := kimiForwardUsageContext{
		component:      "handler.kimi_gateway.messages",
		apiKey:         apiKey,
	...
	for {
		...
		result, err := h.kimiService.ForwardMessages(c.Request.Context(), c, account, forwardBody, kimiRequestID(c))
		releaseOnce()
		if err != nil {
...
func (h *KimiGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	usageCtx := kimiForwardUsageContext{
		component:      "handler.kimi_gateway.chat_completions",
		apiKey:         apiKey,
		subscription:   subscription,
		userID:         subject.UserID,
		reqModel:       reqModel,
	...
	for {
		...
		result, err := h.kimiService.ForwardChatCompletions(c.Request.Context(), c, account, forwardBody, kimiRequestID(c))
		releaseOnce()
		if err != nil {
			if c.Writer.Written() || streamStarted {
				// 响应已提交（流已开始或已写出）：若上游已产生 usage（含客户端中途断开后的 drain 结果）
...
func (h *KimiGatewayHandler) Responses(c *gin.Context) {
	...
	usageCtx := kimiForwardUsageContext{
		component:      "handler.kimi_gateway.responses",
		apiKey:         apiKey,
		subscription:   subscription,
		userID:         subject.UserID,
		reqModel:       reqModel,
	...
	for {
		...
		result, err := h.kimiService.ForwardResponses(c.Request.Context(), c, account, forwardBody, kimiRequestID(c))
		releaseOnce()
		if err != nil {
			if c.Writer.Written() || streamStarted {
				// 响应已提交（流已开始或已写出）：若上游已产生 usage（含客户端中途断开后的 drain 结果）
...
func (h *KimiGatewayHandler) submitUsageRecordTask(parent context.Context, task service.UsageRecordTask) {
	submitUsageRecordTaskWithFallback(
		parent,
		h.usageRecordWorkerPool,
		"handler.kimi_gateway.messages",
		task,
	)
}
...
// kimiForwardUsageContext 聚合一次转发的计费上下文，供成功路径与流中断路径共用同一份 RecordUsage 提交逻辑。
type kimiForwardUsageContext struct {
	component      string
...
func (h *KimiGatewayHandler) submitForwardUsage(c *gin.Context, uc kimiForwardUsageContext, account *service.Account, result *service.ForwardResult) {
...
func (h *KimiGatewayHandler) handleInterruptedForward(c *gin.Context, uc kimiForwardUsageContext, account *service.Account, result *service.ForwardResult, err error, streamStarted bool) {
	if result != nil {
	...
	).Warn("kimi_gateway.stream_interrupted_without_usage", zap.Error(err))
}
