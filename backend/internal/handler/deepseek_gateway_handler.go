...
func (h *DeepSeekGatewayHandler) Messages(c *gin.Context) {
	...
	usageCtx := deepseekForwardUsageContext{
		component:      "handler.deepseek_gateway.messages",
		apiKey:         apiKey,
	...
	for {
		...
		result, err := h.deepseekService.ForwardMessages(c.Request.Context(), c, account, forwardBody, deepseekRequestID(c))
		releaseOnce()
		if err != nil {
...
func (h *DeepSeekGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	usageCtx := deepseekForwardUsageContext{
		component:      "handler.deepseek_gateway.chat_completions",
		apiKey:         apiKey,
		subscription:   subscription,
		userID:         subject.UserID,
		reqModel:       reqModel,
	...
	for {
		...
		result, err := h.deepseekService.ForwardChatCompletions(c.Request.Context(), c, account, forwardBody, deepseekRequestID(c))
		releaseOnce()
		if err != nil {
			if c.Writer.Written() || streamStarted {
				// 响应已提交（流已开始或已写出）：若上游已产生 usage（含客户端中途断开后的 drain 结果）
...
// Responses handles DeepSeek OpenAI-compatible POST /v1/responses requests.
func (h *DeepSeekGatewayHandler) Responses(c *gin.Context) {
	apiKey, ok := middleware2.GetAPIKeyFromContext(c)
	if !ok {
		h.errorResponse(c, http.StatusUnauthorized, "authentication_error", "Invalid API key")
		return
	...
	usageCtx := deepseekForwardUsageContext{
		component:      "handler.deepseek_gateway.responses",
		apiKey:         apiKey,
		subscription:   subscription,
		userID:         subject.UserID,
		reqModel:       reqModel,
	...
	for {
		...
		result, err := h.deepseekService.ForwardResponses(c.Request.Context(), c, account, forwardBody, deepseekRequestID(c))
		releaseOnce()
		if err != nil {
			if c.Writer.Written() || streamStarted {
				// 响应已提交（流已开始或已写出）：若上游已产生 usage（含客户端中途断开后的 drain 结果）
...
func (h *DeepSeekGatewayHandler) submitUsageRecordTask(parent context.Context, task service.UsageRecordTask) {
	submitUsageRecordTaskWithFallback(
		parent,
		h.usageRecordWorkerPool,
		"handler.deepseek_gateway.messages",
		task,
	)
}
...
// deepseekForwardUsageContext 聚合一次转发的计费上下文，供成功路径与流中断路径共用同一份 RecordUsage 提交逻辑。
type deepseekForwardUsageContext struct {
	component      string
...
func (h *DeepSeekGatewayHandler) submitForwardUsage(c *gin.Context, uc deepseekForwardUsageContext, account *service.Account, result *service.ForwardResult) {
...
func (h *DeepSeekGatewayHandler) handleInterruptedForward(c *gin.Context, uc deepseekForwardUsageContext, account *service.Account, result *service.ForwardResult, err error, streamStarted bool) {
	if result != nil {
	...
	).Warn("deepseek_gateway.stream_interrupted_without_usage", zap.Error(err))
}
