...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	channelMapping := h.gatewayService.ResolveRequestChannelMapping(c.Request.Context(), apiKey.GroupID, requestedModel)
	...
	for {
		selection, _, routingModel, err := h.gatewayService.SelectAccountWithSchedulerForMappedCapability(
			c.Request.Context(),
			apiKey.GroupID,
			"",
			sessionHash,
			...
			false,
			false,
			// 网页搜索按次计费，上游不返回 usage/token。
			service.BillingKindWebSearch,
		...
		if err != nil || selection == nil || selection.Account == nil {
			...
			if len(failedAccountIDs) == 0 {
				cls := classifyNoAccountErrorFromGin(c, h.gatewayService, apiKey, err, routingModel, requestedModel, service.PlatformOpenAI)
...
func (h *OpenAIGatewayHandler) recordAlphaSearchUsage(
	...
	h.submitMandatoryUsageRecordTask(c.Request.Context(), func(ctx context.Context) {
		if err := h.gatewayService.RecordUsage(ctx, &service.OpenAIRecordUsageInput{
			...
			SessionID:          sessionID,
			// 网页搜索按次计费：上游不返回 usage/token，口径只能由入口给出。
			BillingKind:        service.BillingKindWebSearch,
