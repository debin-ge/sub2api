...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	if err := service.ValidateUniqueOpenAIServiceTierField(body); err != nil {
		h.errorResponse(c, http.StatusBadRequest, "invalid_request_error", err.Error())
		return
	}
				...
				if errors.As(err, &failoverErr) {
					...
					if capacityRecovery.IsCapacityFailure(failoverErr) {
						if retryCount, retryDelay, ok := capacityRecovery.BeginSameAccountRetry(account.ID, failoverErr); ok {
							reqLog.Warn("openai_chat_completions.capacity_same_account_retry",
								zap.Int64("account_id", account.ID),
...
func resolveOpenAIUpstreamEndpoint(c *gin.Context, account *service.Account, result *service.OpenAIForwardResult) string {
	...
	if account != nil && account.Type == service.AccountTypeAPIKey &&
		!service.IsAzureOpenAIAPIKeyAccount(account) &&
