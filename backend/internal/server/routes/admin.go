...
func registerResellerRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	{
		reseller.GET("/upstream-balance", h.Admin.Reseller.GetUpstreamBalance)
	}
}
...
func registerOpsRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	ops := admin.Group("/ops")
	{
		...
		ops.GET("/api-key-email-automation/health", h.Admin.Ops.GetAPIKeyEmailAutomationHealth)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.GET("/vip-reconcile/preview", h.Admin.User.PreviewVIPReconcile)
		users.POST("/vip-reconcile/jobs", h.Admin.User.CreateVIPReconcileJob)
		users.GET("/vip-reconcile/jobs/:job_id", h.Admin.User.GetVIPReconcileJob)
		users.GET("/:id", h.Admin.User.GetByID)
		users.PUT("/:id/vip-mode", h.Admin.User.UpdateVIPMode)
		users.GET("/:id/vip-audit", h.Admin.User.ListVIPAudit)
		users.GET("/:id/group-catalog", h.Admin.User.GetGroupCatalog)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.GET("/access-graph/scan", h.Admin.Group.ScanAccessGraph)
