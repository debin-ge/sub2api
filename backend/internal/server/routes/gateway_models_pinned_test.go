...
func (r *pinnedModelsRoutesRepository) ListSchedulableByGroupIDAndPlatform(_ context.Context, _ int64, platform string) ([]service.Account, error) {
...
type pinnedModelsRoutesGroupRepository struct {
	service.GroupRepository
	group service.Group
}

func (r *pinnedModelsRoutesGroupRepository) GetByID(_ context.Context, id int64) (*service.Group, error) {
	if r.group.ID != id {
...
func (u *pinnedModelsRoutesUpstream) Discover(context.Context, *service.Account) ([]string, error) {
	u.ordinaryCalls.Add(1)
	return []string{"ordinary-upstream-model"}, nil
}
...
func TestGatewayRoutesPinnedModelsDispatchesOrdinaryAndCodexRequests(t *testing.T) {
	...
	catalog := service.NewModelCatalogService(
		repo,
		&pinnedModelsRoutesGroupRepository{group: *group},
		nil,
		upstream,
		config.ModelCatalogConfig{},
	)
	h := &handler.Handlers{
		Gateway:       handler.NewGatewayHandler(nil, s, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, cfg, nil, catalog),
