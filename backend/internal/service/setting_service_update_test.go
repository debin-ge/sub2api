...
func TestSettingService_UpdateSettings_RegistrationEmailSuffixBlacklist_Normalized(t *testing.T) {
	...
	err := svc.UpdateSettings(context.Background(), &SystemSettings{
		RegistrationEmailSuffixBlacklist: []string{"example.com", "@EXAMPLE.com", " @foo.bar ", "*.EDU.CN"},
	})
	require.NoError(t, err)
	require.Equal(t, `["@example.com","@foo.bar","*.edu.cn"]`, repo.updates[SettingKeyRegistrationEmailSuffixBlacklist])
}
...
func TestSettingService_LoadForwardedClientIPSettingsMigration(t *testing.T) {
	tests := []struct {
		...
		{
			// 迁移不再把已持久化的 false 回翻成 true：未配置可信代理时只告警，保持关闭。
			name:                   "legacy false without proxy config stays disabled",
...
func TestSettingService_LoadForwardedClientIPSettingsWriteFailureUsesComputedMode(t *testing.T) {
	tests := []struct {
		...
		// 写入失败时仍按计算结果生效；持久化的 false 不再被回翻为 true。
		{name: "legacy false without proxy config stays disabled", wantEnabled: false},
