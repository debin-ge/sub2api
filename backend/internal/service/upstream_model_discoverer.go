...
func (d *UpstreamModelDiscoverer) buildGrokUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	case AccountTypeOAuth:
		if d.grokTokenProvider == nil {
			return nil, newUpstreamModelSyncConfigError("Grok token provider is not configured", nil)
		}
		accessToken, err := d.grokTokenProvider.GetAccessTokenForManualTest(ctx, account)
		if err != nil {
		...
		validator, err := grokBaseURLValidator(account, d.cfg)
		if err != nil {
			return nil, newUpstreamModelSyncConfigError("Invalid Grok base URL", err)
		}
...
func (d *UpstreamModelDiscoverer) fetchAntigravityOAuthUpstreamModels(ctx context.Context, account *Account) ([]string, error) {
	...
	modelsResp, _, err := client.FetchAvailableModels(
		...
		accessToken,
		strings.TrimSpace(account.GetCredential("project_id")),
		resolveModelsListReadLimit(d.cfg),
	)
	if err != nil {
...
func grokUpstreamModelEntryID(entry upstreamModelEntry) string {
	...
	candidates = append(candidates, entry.Name, entry.Slug)
