...
// GetModelPricingStrict 供准入守卫/实时后扣/补偿结算使用，不接受跨模型推断
// 出来的价格（见其函数注释）；未知 deepseek-* 的 flash 兜底属于跨模型推断，
// Strict 变体必须继续 fail-closed。
...
// 非 Strict 的 GetModelPricing 允许跨模型推断：未知 deepseek-* 统一按 flash
// 价兜底，避免计费中断。
