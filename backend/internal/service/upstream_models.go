...
func (s *AccountTestService) SyncUpstreamModelCatalog(ctx context.Context, account *Account) (*UpstreamModelCatalog, error) {
	...
	source := "upstream"
	metadataIncomplete := upstreamCatalogNeedsRegistry(capabilityIDs, catalog.Metadata)
	// Domestic provider model endpoints are authoritative for their own model
	// IDs and are not represented reliably in the public models.dev registry.
	// Avoid a second request through the provider credential path (and avoid
	// treating a registry miss as a sync warning) for those accounts.
	_, hasDomesticProviderCatalog := GetProviderGatewayCapabilities(upstreamModelSyncPlatform(account))
	if metadataIncomplete && !hasDomesticProviderCatalog {
...
func (s *AccountTestService) fetchUpstreamModelList(ctx context.Context, account *Account) ([]string, []byte, error) {
	...
	if discoverer, ok := s.modelDiscoverer.(interface {
		DiscoverWithBody(context.Context, *Account) ([]string, []byte, error)
	}); ok {
		return discoverer.DiscoverWithBody(ctx, account)
	}
	if s.modelDiscoverer != nil {
		models, err := s.modelDiscoverer.Discover(ctx, account)
	...
	proxyURL := metadataUpstreamModelsProxyURL(account)
	...
	extractModels := metadataExtractUpstreamModelIDs
	if account.IsGrok() {
		extractModels = metadataExtractGrokUpstreamModelIDs
	}
...
func (s *AccountTestService) buildGrokUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, metadataBuildOpenAIModelsURL(normalizedBaseURL), nil)
	if err != nil {
...
func (s *AccountTestService) buildAnthropicUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, metadataBuildV1ModelsURL(normalizedBaseURL), nil)
	if err != nil {
...
func (s *AccountTestService) buildGeminiUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodGet, metadataBuildGeminiModelsURL(normalizedBaseURL), nil)
	if err != nil {
...
func (s *AccountTestService) fetchAntigravityOAuthUpstreamModels(ctx context.Context, account *Account) ([]string, error) {
	...
	client, err := antigravity.NewClient(metadataUpstreamModelsProxyURL(account))
	if err != nil {
...
func metadataUpstreamModelsProxyURL(account *Account) string {
...
func metadataBuildV1ModelsURL(base string) string {
...
func metadataBuildOpenAIModelsURL(base string) string {
	return buildOpenAIEndpointURL(base, "/v1/models")
}

func metadataBuildGeminiModelsURL(base string) string {
	normalized := strings.TrimRight(strings.TrimSpace(base), "/")
...
type metadataUpstreamModelEntry struct {
	ID           string          `json:"id"`
...
type metadataUpstreamModelEntryMetadata struct {
	ID           string `json:"id"`
	Slug         string `json:"slug"`
	Model        string `json:"model"`
	ModelID      string `json:"modelId"`
...
func metadataExtractUpstreamModelIDs(body []byte) ([]string, error) {
	return metadataExtractUpstreamModelIDsWithSelector(body, metadataUpstreamModelEntryID)
}

func metadataExtractGrokUpstreamModelIDs(body []byte) ([]string, error) {
	return metadataExtractUpstreamModelIDsWithSelector(body, metadataGrokUpstreamModelEntryID)
}
...
func extractUpstreamModelCatalog(body []byte, grok bool) ([]string, map[string]UpstreamModelMetadata, error) {
	...
	selectID := metadataUpstreamModelEntryID
	if grok {
		selectID = metadataGrokUpstreamModelEntryID
	}
	...
	for _, raw := range entries {
		...
		modelID := strings.TrimSpace(selectID(capability.metadataUpstreamModelEntry))
		if modelID == "" {
			continue
		}
	...
	return metadataDedupeAndSortModelIDs(models), metadata, nil
}
...
func metadataExtractUpstreamModelIDsWithSelector(body []byte, selectID func(metadataUpstreamModelEntry) string) ([]string, error) {
	var response struct {
		Data   []metadataUpstreamModelEntry `json:"data"`
		Models []metadataUpstreamModelEntry `json:"models"`
	}
	if err := json.Unmarshal(body, &response); err != nil {
		var arrayResponse []metadataUpstreamModelEntry
		...
		return metadataDedupeAndSortModelIDs(models), nil
	}
...
func metadataUpstreamModelEntryID(entry metadataUpstreamModelEntry) string {
...
func metadataGrokUpstreamModelEntryID(entry metadataUpstreamModelEntry) string {
	candidates := []string{
		entry.Model,
		entry.ModelID,
		entry.ModelIDSnake,
	...
	if len(entry.Meta) > 0 {
		var meta metadataUpstreamModelEntryMetadata
...
func metadataDedupeAndSortModelIDs(models []string) []string {
