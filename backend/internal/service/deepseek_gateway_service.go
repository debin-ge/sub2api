...
// handleStreamingChatCompletionsResponse 透传 DeepSeek Chat Completions SSE 流；返回值约定见 forwardCompatSSEStream。
func (s *DeepSeekGatewayService) handleStreamingChatCompletionsResponse(resp *http.Response, c *gin.Context, originalModel string, upstreamModel string, start time.Time, drain *compatStreamDrain) (*ForwardResult, error) {
	return forwardCompatSSEStream(resp, c, s.responseHeaderFilter, "deepseek", parseDeepSeekOpenAIStreamingUsage, originalModel, upstreamModel, start, drain)
}
