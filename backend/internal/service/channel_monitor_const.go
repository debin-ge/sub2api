...
const (
	...
	// providerOpenCodeChatPath OpenCode2API-compatible Chat Completions 路径。
	providerOpenCodeChatPath = "/v1/chat/completions"

	// MonitorProviderOpenAI 等 provider 字符串常量（也是 ent enum 的实际值）。
	// antigravity/kimi/zhipu/deepseek 为配额模式引入：antigravity 无探活
	// adapter（仅配额）；zhipu 走 OpenAI 兼容探活。本地扩展平台
	// minimax/glm/windsurf/opencode 保留原探活路径。
	...
	MonitorProviderDeepSeek    = MonitorProviderDeepseek
...
var (
	...
	ErrChannelMonitorInvalidProvider = infraerrors.BadRequest(
		"CHANNEL_MONITOR_INVALID_PROVIDER", "provider must be one of openai/anthropic/gemini/grok/antigravity/kimi/zhipu/deepseek/minimax/glm/windsurf/opencode",
	)
