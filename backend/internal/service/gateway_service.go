...
func (s *GatewayService) GetAvailableModels(ctx context.Context, groupID *int64, platform string) []string {
	...
	if len(accounts) == 0 && platform == PlatformDeepseek {
		return s.cacheAvailableModels(cacheKey, NewGatewayModelListProvider(GatewayModelListOptions{
			IncludeAliases: includeDomesticModelAliasesInModels(s.cfg),
		}).ModelsForProvider(platform, nil))
	}

	// DeepSeek model mappings are an explicit group catalog. Keep the same
	// mapping-only behavior as other routed providers so a configured alias does
	// not silently widen the Codex/model list to every official SKU.
	if hasDomesticCapabilities && platform != PlatformDeepseek {
		return s.cacheAvailableModels(cacheKey, NewGatewayModelListProvider(GatewayModelListOptions{
			IncludeAliases: includeDomesticModelAliasesInModels(s.cfg),
