...
func (s *PaymentService) checkPaidWithOptions(ctx context.Context, o *dbent.PaymentOrder, opts checkPaidOptions) string {
	...
	finishProviderCall := servertiming.ObserveDependency(queryCtx, "payment")
	...
	if cp, ok := prov.(payment.CancelableProvider); ok {
		queryCtx := paymentProviderQueryContext(ctx, o, prov)
		finishProviderCall := servertiming.ObserveDependency(queryCtx, "payment")
		err := cp.CancelPayment(queryCtx, queryRef)
		finishProviderCall()
		if err != nil {
...
// ReconcilePendingWxpayOrders is retained for compatibility with older callers.
