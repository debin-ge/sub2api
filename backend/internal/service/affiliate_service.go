...
type AffiliateDetail struct {
	...
	// EffectiveRegistrationRewardAmount is the reward currently available for a
	// newly-created invitee after subtracting this inviter's prior registration
	// and recharge rebates from the global inviter cap. Historical rewards keep
	// their ledger snapshot.
...
type AffiliateRepository interface {
	...
	BindInviterAndGrantRegistrationReward(ctx context.Context, inviteeUserID int64, affiliateCode string, rewardAmount float64, inviterTotalCap float64, freezeHours int) (*AffiliateRegistrationRewardResult, error)
	// AccrueQuota atomically applies the requested recharge rebate without
	// exceeding the inviter's global registration + recharge cap. It returns
	// the actual amount applied after cap truncation.
	AccrueQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, inviterTotalCap float64, freezeHours int, sourceOrderID *int64) (float64, error)
	// GetAccruedRebateFromInvitee returns all rebate generated by one invitee,
	// including registration rewards and recharge rebates.
	GetAccruedRebateFromInvitee(ctx context.Context, inviterID, inviteeUserID int64) (float64, error)
	// GetTotalRebateForInviter returns all registration rewards and recharge
	// rebates earned by an inviter across every invitee.
	GetTotalRebateForInviter(ctx context.Context, inviterID int64) (float64, error)
...
func (s *AffiliateService) GetAffiliateDetail(ctx context.Context, userID int64) (*AffiliateDetail, error) {
	...
	effectiveRegistrationRewardAmount, err := s.effectiveRegistrationRewardAmount(ctx, userID)
	if err != nil {
		return nil, err
	}
	return &AffiliateDetail{
		...
		EffectiveRegistrationRewardAmount: effectiveRegistrationRewardAmount,
		Invitees:                          invitees,
	}, nil
}
...
func (s *AffiliateService) BindInviterAndGrantRegistrationReward(ctx context.Context, userID int64, rawCode string) (*AffiliateRegistrationRewardResult, error) {
	...
	inviterTotalCap := AffiliateRebatePerInviteeCapDefault
	freezeHours := 0
	if s.settingService != nil {
		inviterTotalCap = s.settingService.GetAffiliateRebatePerInviteeCap(ctx)
		freezeHours = s.settingService.GetAffiliateRebateFreezeHours(ctx)
	}
	result, err := s.repo.BindInviterAndGrantRegistrationReward(
		ctx,
		userID,
		code,
		rewardAmount,
...
func (s *AffiliateService) effectiveRegistrationRewardAmount(ctx context.Context, inviterID int64) (float64, error) {
	rewardAmount := s.registrationRewardAmount(ctx)
	if rewardAmount <= 0 || s == nil || s.settingService == nil {
	...
	totalRebate, err := s.repo.GetTotalRebateForInviter(ctx, inviterID)
	if err != nil {
		return 0, err
	}
	remaining := roundTo(inviterTotalCap-totalRebate, 8)
	if remaining <= 0 {
		return 0, nil
	}
...
func (s *AffiliateService) AccrueInviteRebateForOrder(ctx context.Context, inviteeUserID int64, baseRechargeAmount float64, sourceOrderID *int64) (float64, error) {
	...
	appliedAmount, err := s.repo.AccrueQuota(
		ctx,
