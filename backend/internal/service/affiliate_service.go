...
type AffiliateDetail struct {
	...
	// EffectiveRegistrationRewardAmount is the maximum current reward for a
	// newly-created invitee after applying the per-invitee cap. Historical
	// rewards keep their ledger snapshot.
...
type AffiliateRepository interface {
	...
	BindInviterAndGrantRegistrationReward(ctx context.Context, inviteeUserID int64, affiliateCode string, rewardAmount float64, perInviteeCap float64, freezeHours int) (*AffiliateRegistrationRewardResult, error)
	// AccrueQuota atomically applies the requested recharge rebate without
	// exceeding the shared registration + recharge cap. It returns the actual
	// amount applied after cap truncation.
	AccrueQuota(ctx context.Context, inviterID, inviteeUserID int64, amount float64, perInviteeCap float64, freezeHours int, sourceOrderID *int64) (float64, error)
	// GetAccruedRebateFromInvitee returns all cap-eligible rebate generated by
	// one invitee, including registration rewards and recharge rebates.
...
func (s *AffiliateService) GetAffiliateDetail(ctx context.Context, userID int64) (*AffiliateDetail, error) {
	...
	return &AffiliateDetail{
		...
		EffectiveRegistrationRewardAmount: s.effectiveRegistrationRewardAmount(ctx),
		Invitees:                          invitees,
	}, nil
}
...
func (s *AffiliateService) BindInviterAndGrantRegistrationReward(ctx context.Context, userID int64, rawCode string) (*AffiliateRegistrationRewardResult, error) {
	...
	perInviteeCap := AffiliateRebatePerInviteeCapDefault
	freezeHours := 0
...
func (s *AffiliateService) effectiveRegistrationRewardAmount(ctx context.Context) float64 {
	rewardAmount := s.registrationRewardAmount(ctx)
	if rewardAmount <= 0 || s == nil || s.settingService == nil {
		return rewardAmount
	}
	perInviteeCap := s.settingService.GetAffiliateRebatePerInviteeCap(ctx)
	if perInviteeCap > 0 && rewardAmount > perInviteeCap {
		return roundTo(perInviteeCap, 8)
	}
	return rewardAmount
}
