...
type OpenAIForwardResult struct {
	...
	// FallbackRequestBody is retained only until usage recording and is never persisted.
	FallbackRequestBody []byte
	// FallbackSemanticOutput contains response text/tool arguments used only for
...
// ResolveChannelMappingAndRestrict preserves the upstream compatibility
// contract while keeping this fork's separation of concerns: mapping is
// resolved here and the pricing restriction remains enforced by the scheduler.
func (s *OpenAIGatewayService) ResolveChannelMappingAndRestrict(ctx context.Context, groupID *int64, model string) (ChannelMappingResult, bool) {
	mapping := s.ResolveRequestChannelMapping(ctx, groupID, model)
	return mapping, s.checkChannelPricingRestriction(ctx, groupID, model)
}
