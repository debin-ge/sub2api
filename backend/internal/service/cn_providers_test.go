...
func TestBuildUpstreamModelsRequest_CNProviders(t *testing.T) {
	...
	discoverer := &UpstreamModelDiscoverer{cfg: &config.Config{}}
	cases := []struct {
		name     string
		platform string
		mode     string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := discoverer.buildUpstreamModelsRequest(context.Background(), account)
...
func TestAdaptiveProtocolBaseURLOverrides(t *testing.T) {
	...
	account := &Account{Platform: PlatformDeepseek, Type: AccountTypeAPIKey, Credentials: map[string]any{
		...
		"base_url_openai":    "https://legacy-openai.example.com",
		"base_url_anthropic": "https://legacy-anthropic.example.com",
...
func TestAdaptiveProtocolBaseURLFallsBackToLegacyProtocolFields(t *testing.T) {
	t.Parallel()

	account := &Account{Platform: PlatformZhipu, Type: AccountTypeAPIKey, Credentials: map[string]any{
		"api_protocol":       APIProtocolAdaptive,
		"base_url":           "https://legacy-common.example.com",
		"base_url_openai":    "https://legacy-openai.example.com/",
		"base_url_anthropic": "https://legacy-anthropic.example.com/",
		"api_base_urls": map[string]any{
	...
	require.Equal(t, "https://legacy-openai.example.com", account.GetCNProtocolBaseURL(APIProtocolChatCompletions))
	require.Equal(t, "https://legacy-anthropic.example.com", account.GetCNProtocolBaseURL(APIProtocolAnthropic))
}
