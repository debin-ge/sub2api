...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyRegistrationRateLimitPerIP:                strconv.Itoa(RegistrationRateLimitPerIPDefault),
		...
		SettingKeySiteLogo:                                  "",
		SettingKeyContactQRCode:                             "",
		SettingKeyContactQRCodeEnabled:                      "false",
		...
		SettingKeyResellerUpstreamEndpoint:                  "",
		SettingKeyResellerUpstreamAPIKey:                    "",
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result := &SystemSettings{
		...
		ContactQRCode:                          settings[SettingKeyContactQRCode],
	...
	// Reseller 子站配置：数据库设置优先，空 endpoint/key 回退 config.yaml/env，
	// 保留旧部署未迁移到数据库设置时的运行行为。
	...
	if value := strings.TrimSpace(settings[SettingKeyResellerUpstreamEndpoint]); value != "" {
		result.ResellerUpstreamEndpoint = value
	} else {
		result.ResellerUpstreamEndpoint = strings.TrimSpace(resellerBase.UpstreamEndpoint)
	}
