...
func TestDefaultPricingUsesCurrentCodexAutoReviewBaseRates(t *testing.T) {
	...
	require.NoError(t, err)
	require.NotNil(t, got)
	require.InDelta(t, 0.2e-6, got.InputPricePerToken, 1e-12)
	require.InDelta(t, 1.2e-6, got.OutputPricePerToken, 1e-12)
	require.InDelta(t, 0.02e-6, got.CacheReadPricePerToken, 1e-12)
	...
	require.Zero(t, got.OutputPricePerTokenPriority)
	require.Zero(t, got.CacheReadPricePerTokenPriority)
...
// F1：长上下文价格的两侧倍率显式给出时，1.0 侧不得被当成免费（0）。
		...
		svc := NewBillingService(&config.Config{}, newStubPricingServiceFromJSON(t, `{
			"partial-in": {"litellm_provider": "openai", "mode": "chat",
				...
				"cache_read_input_token_cost": 2e-07,
					...
					"long_context_output_cost_multiplier": 1.0}
		}`))
	...
	t.Run("only output multiplier", func(t *testing.T) {
		svc := NewBillingService(&config.Config{}, newStubPricingServiceFromJSON(t, `{
			"partial-out": {"litellm_provider": "openai", "mode": "chat",
				...
				"cache_read_input_token_cost": 2e-07,
					"long_context_input_token_threshold": 272000,
					"long_context_input_cost_multiplier": 1.0,
					"long_context_output_cost_multiplier": 1.5}
		}`))
