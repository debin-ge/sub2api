...
type SettingService struct {
	...
	// in-flight read and generation; repository I/O is never performed while
	// it is held. The atomic LKG uses 0 for unknown, 1 for false, and 2 for true.
...
// 返回 0 表示永久有效。
// GetAffiliateRebatePerInviteeCap 返回邀请人跨全部受邀用户的累计返利上限。
// 方法名和配置键为兼容既有客户端而保留。
// 返回 0 表示无上限。
