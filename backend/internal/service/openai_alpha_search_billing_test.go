...
func TestCalculateOpenAIRecordUsageCostWebSearchPerCall(t *testing.T) {
	...
	cost, err := svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, BillingKindUnspecified, 3.0, 1.0, 1.0, 2.0, UsageTokens{}, "", false)
	...
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, BillingKindUnspecified, 1.0, 1.0, 1.0, 1.0, UsageTokens{}, "", false)
	require.NoError(t, err)
	...
	_, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, BillingKindUnspecified, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.Error(t, err)

	// 入口显式给出 web_search 口径后，不再依赖上游回不回 WebSearchCalls：
	// 该路由本就不返回 usage，靠计数反推等于把漏回字段变成"换一张价格表"。
	cost, err = svc.calculateOpenAIRecordUsageCost(context.Background(), result, apiKey, []string{"gpt-5.6-sol"}, BillingKindWebSearch, 1.0, 1.0, 1.0, 1.0, UsageTokens{InputTokens: 10}, "", false)
	require.NoError(t, err)
