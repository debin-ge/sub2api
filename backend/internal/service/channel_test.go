...
func TestSupportedModels_ExactMappingTargetMissingFromPricing(t *testing.T) {
	...
	// （等待 ListAvailable 阶段的全局 configured pricing catalog 回落填充）。
	ch := &Channel{
		ModelPricing: []ChannelModelPricing{
	...
	require.Nil(t, got[0].Pricing, "target 在渠道定价中缺失时不虚假填充，留给 ListAvailable 走 configured pricing catalog 回落")
