...
func TestIntervalExplicitPriceTakesPrecedenceOverMultiplier(t *testing.T) {
	pricing := intervalToModelPricing(&PricingInterval{
		...
		InputMultiplier: pricingMultiplier(2),
	}, &ModelPricing{InputPricePerToken: 5}, false, nil)
...
func TestMultiplierOnlyPricingPreservesDeepSeekOfficialTimeProvenance(t *testing.T) {
	billing := NewBillingService(&config.Config{}, nil)
	fastMultiplier := 2.5
	pricing, err := billing.GetModelPricingWithChannelForPlatform(
		PlatformDeepSeek,
		"deepseek-v4-flash",
		&ChannelModelPricing{FastMultiplier: &fastMultiplier},
	)
	require.NoError(t, err)
	require.True(t, pricing.OfficialTimePricing)

	intervalMultiplier := 2.0
	multiplied := intervalToModelPricing(
		&PricingInterval{InputMultiplier: &intervalMultiplier},
		pricing,
		false,
		nil,
	)
	require.True(t, multiplied.OfficialTimePricing)

	explicitPrice := 0.001
	explicit := intervalToModelPricing(
		&PricingInterval{InputPrice: &explicitPrice, InputMultiplier: &intervalMultiplier},
		pricing,
		false,
		nil,
	)
	require.False(t, explicit.OfficialTimePricing)
	require.InDelta(t, explicitPrice, explicit.InputPricePerToken, 1e-12)
}
