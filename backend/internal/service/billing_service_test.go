...
func TestGetModelPricing_FallbackWarnLoggedOncePerModel(t *testing.T) {
	...
	// glm-5.2 不在 LiteLLM,命中硬编码兜底价 → 触发 fallback warn。
	for i := 0; i < 5; i++ {
...
// 回归:glm-5.2 有独立兜底价($1.4/$4.4,z.ai 官方 SKU 价),不得再降级到 glm-5。
// "glm-5.2" 含子串 "glm-5",匹配顺序一旦颠倒就会误算成 $1.0/$3.2。
...
func TestCalculateCost_OpenAIGPT54LongContextMarkerRequiresActualCostIncrease(t *testing.T) {
	...
	cost, err := svc.calculateCostInternalForPlatform(
		"",
		"gpt-5.4-2026-03-05",
