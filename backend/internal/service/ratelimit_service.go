...
// usage limit)"），窗口重置点与本次限制无关，不能拿来做小时级冷却。只有聚合 reset
// 而没有状态时仍走历史聚合窗口逻辑，不能仅凭状态缺失推断为突发限制。
func isAnthropicBurst429(headers http.Header) bool {
	...
	return strings.EqualFold(strings.TrimSpace(headers.Get("anthropic-ratelimit-unified-status")), "allowed")
}
