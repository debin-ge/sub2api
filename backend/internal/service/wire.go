...
func ProvidePricingService(
	...
	svc.SetOverrideDependencies(overrideStore, overrideCache)
...
func ProvideAccountUsageService(
	...
	if cfg != nil && cfg.Radar.SourceHardRetentionDays > 0 {
		service.radarSnapshotHardRetention = time.Duration(cfg.Radar.SourceHardRetentionDays) * 24 * time.Hour
	}
	return service
}
...
func ProvideConcurrencyService(cache ConcurrencyCache, accountRepo AccountRepository, cfg *config.Config) *ConcurrencyService {
	...
	// 注意跳过的代价：周期清理只按分数裁剪过期成员、不认进程前缀，残留槽位
	// 只能等 slot TTL（默认 30 分钟）自然过期，期间账号并发虚高甚至不被调度。
