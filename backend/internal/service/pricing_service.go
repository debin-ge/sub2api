...
func (s *PricingService) lookupModelPricingForPlatforms(platforms []string, modelName string, allowInference bool) (*ModelPriceEntry, string) {
	...
	if strings.HasPrefix(lookupCandidates[0], "gpt-") {
		if pricing := s.matchOpenAIModel(lookupCandidates[0]); inferredOpenAIPricingCandidateAllowed(lookupCandidates[0], pricing) {
			return pricing, basePricingPlatform(platforms)
		}
	}
...
func inferredOpenAIPricingCandidateAllowed(model string, pricing *LiteLLMModelPricing) bool {
	if inferredPricingCandidateAllowed(pricing) {
		return true
	}
	return pricing != nil && isOpenAIImageGenerationModel(model) &&
		(pricing.OutputCostPerImage > 0 || pricing.OutputCostPerImageToken > 0)
}
