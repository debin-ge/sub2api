...
func TestConfirmManualRefundMarksRefundedAndDeductsLocally(t *testing.T) {
	ctx := context.Background()
	client := newPaymentConfigServiceTestClient(t)

	user, err := client.User.Create().
		SetEmail("manual-refund@example.com").
		SetPasswordHash("hash").
		SetUsername("manual-refund-user").
		Save(ctx)
	require.NoError(t, err)

	inst, err := client.PaymentProviderInstance.Create().
		SetProviderKey(payment.TypeWise).
		SetName("wise-manual-refund").
		SetConfig("{}").
	...
	order, err := client.PaymentOrder.Create().
		...
		SetPayAmount(50).
		SetFeeRate(0).
		SetRechargeCode("MANUAL-REFUND-ORDER").
		SetOutTradeNo("sub2_manual_refund").
		SetPaymentType(payment.TypeWise).
		SetPaymentTradeNo("wise-tx-1").
		SetOrderType(payment.OrderTypeBalance).
		SetStatus(OrderStatusRefunding).
		SetRefundAmount(50).
