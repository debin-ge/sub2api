...
func TestGatewayProfitControlSelectionCarriesGateToHandlerContext(t *testing.T) {
	...
	selection, err := svc.newSelectionResult(gateCtx, &group.ID, "", &expensive, true, nil, nil)
	require.NoError(t, err)
	...
	// 无门选号不携带门，重放为无操作。
	plain, err := svc.newSelectionResult(context.Background(), &group.ID, "", &expensive, true, nil, nil)
	require.NoError(t, err)
