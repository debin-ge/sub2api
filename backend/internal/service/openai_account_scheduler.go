...
func (s *defaultOpenAIAccountScheduler) Select(
	...
	if previousResponseID != "" && NormalizeOpenAICompatiblePlatform(CanonicalCNPlatform(req.Platform)) == PlatformOpenAI &&
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || CanonicalCNPlatform(account.Platform) != NormalizeOpenAICompatiblePlatform(CanonicalCNPlatform(req.Platform)) || !account.IsOpenAICompatible() || !account.IsSchedulable() {
		clearBinding()
		return nil, false, nil
	}
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		if CanonicalCNPlatform(account.Platform) != NormalizeOpenAICompatiblePlatform(CanonicalCNPlatform(req.Platform)) || !account.IsOpenAICompatible() {
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, billingKind)
	...
	if NormalizeOpenAICompatiblePlatform(CanonicalCNPlatform(platform)) != PlatformOpenAI {
		return selection, decision, err
	}
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, billingKind)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	platform = NormalizeOpenAICompatiblePlatform(CanonicalCNPlatform(platform))
...
func openAIQuotaHeadroomFactor(account *Account, now time.Time) float64 {
	...
	if window5h.hasUsed && !window5h.reset && !codexWindowMarkedUnavailable(account.Extra, openAICodex5hAvailableExtraKey) {
