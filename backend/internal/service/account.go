...
func (a *Account) IsZhipu() bool {
	return a != nil && (a.Platform == PlatformZhipu || a.Platform == PlatformGLM)
}
...
// IsCNProvider 报告是否为国产 OpenAI 兼容供应商（kimi/zhipu/deepseek，含本地 glm 别名）。
...
func (a *Account) GetOpenAIBaseURL() string {
	...
	if a.IsOpenCodeGo() && a.IsAdaptiveAPIProtocol() {
	...
	if a.Type == AccountTypeAPIKey || a.Type == AccountTypeUpstream {
		...
		if baseURL := strings.TrimSpace(a.GetCredential("base_url_openai")); baseURL != "" {
			return baseURL
		}
	}
...
func (a *Account) GetAccountMode() string {
	...
	// 本地历史 glm 账号没有 account_mode，一律按 Coding Plan 处理。
...
func (a *Account) GetAnthropicProtocolBaseURL() string {
	...
	if a.Type == AccountTypeAPIKey || a.Type == AccountTypeUpstream {
		...
		if baseURL := strings.TrimSpace(a.GetCredential("base_url_anthropic")); baseURL != "" {
			return baseURL
		}
	}
	switch CanonicalCNPlatform(a.Platform) {
...
func (a *Account) ResolveOpenAIResponsesWebSocketV2Mode(defaultMode string) string {
	...
	// setup-token 与 oauth 共用 oauth 前缀键（前端对两类账号统一写 openai_oauth_* 字段）
	if a.IsOpenAIOAuthLike() {
...
// IsCodexCLIOnlyEnabled 返回 OpenAI OAuth/SetupToken 账号是否启用"仅允许 Codex 官方客户端"。
// 字段：accounts.extra.codex_cli_only。
// 字段缺失或类型不正确时，按 false（关闭）处理。
func (a *Account) IsCodexCLIOnlyEnabled() bool {
	if a == nil || !a.IsOpenAI() || !a.IsOAuth() || a.Extra == nil {
		return false
	}
...
// IsCodexCLIOnlyClaudeCodeAllowed reports whether this Codex-only account
// explicitly opts in to the narrow Claude Code plugin signature. The stored
// list is intentionally a set of named presets; arbitrary matching rules are
// not accepted at account scope.
func (a *Account) IsCodexCLIOnlyClaudeCodeAllowed() bool {
	if !a.IsCodexCLIOnlyEnabled() || a.Extra == nil {
		return false
	}
	raw := a.Extra["codex_cli_only_allowed_clients"]
	switch values := raw.(type) {
	case []string:
		for _, value := range values {
			if strings.EqualFold(strings.TrimSpace(value), "claude_code") {
				return true
			}
		}
	case []any:
		for _, value := range values {
			...
			if ok && strings.EqualFold(strings.TrimSpace(client), "claude_code") {
				return true
			}
		}
	}
