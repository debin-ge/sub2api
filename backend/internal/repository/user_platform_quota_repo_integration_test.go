...
// TestBatchSnapshotUsage_OverwriteMultiKey 验证 BatchSnapshotUsage 的窗口内单调覆盖语义：
//  1. 首批写入 2 条既有行（不同 user），验证 daily 等于首批值；
//  2. 对同一 key、同一窗口传更大值，验证 daily 等于新值（单调合并，非累加）。
...
// TestBatchSnapshotUsage_StaleSameWindowSnapshotCannotRollbackDurableBilling 验证同窗口内
// GREATEST 单调合并：一个在 durable billing 事务提交前捕获的陈旧 flusher 快照，不能在事务
// 提交后把已落库的更大值回滚为较小值。
