...
func TestAPIKeyRotationRepositoryListDueFiltersPendingAndDisabledKeys(t *testing.T) {
	...
	now := time.Now().UTC()
	mock.ExpectQuery(`(?s)FROM api_keys k.*status IN \('active', 'quota_exhausted', 'expired'\).*NOT EXISTS.*notification_email_outbox`).
...
func TestAPIKeyRotationRepositoryAtomicallyUpdatesAndEnqueues(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("WITH updated AS (")+`(?s).*UPDATE api_keys.*status = CASE WHEN status = 'expired' THEN 'active' ELSE status END.*INSERT INTO notification_email_outbox.*SELECT rotation_version FROM inserted`).
