...
// BatchSnapshotUsage 用一条多行 UPDATE ... FROM (VALUES ...) 把整批 Redis usage 快照写入既有活跃行。
// 只命中 deleted_at IS NULL 的既有行：行不存在（未配置限额，等价于不限额，与
// BulkInsertInitial/UpsertForUser 的"三档全空即未配置"语义一致）或已软删的
// (user, platform) 直接跳过，不建行。
...
// 另注:启用 flusher 后,本 UPDATE 与 admin 直写 DB(ResetExpiredWindow/UpsertForUser)仍存在有限竞态,
	...
	for start := 0; start < len(snapshots); start += batchRows {
		...
		_, _ = sb.WriteString(
			"UPDATE user_platform_quotas AS q SET" +
				...
				"    WHEN q.daily_window_start IS NULL OR v.daily_window_start > q.daily_window_start THEN v.daily_usage_usd" +
				"    WHEN v.daily_window_start = q.daily_window_start THEN GREATEST(q.daily_usage_usd, v.daily_usage_usd)" +
				"    ELSE q.daily_usage_usd END," +
				"  weekly_usage_usd = CASE" +
				"    WHEN q.weekly_window_start IS NULL OR v.weekly_window_start > q.weekly_window_start THEN v.weekly_usage_usd" +
				"    WHEN v.weekly_window_start = q.weekly_window_start THEN GREATEST(q.weekly_usage_usd, v.weekly_usage_usd)" +
				"    ELSE q.weekly_usage_usd END," +
				"  monthly_usage_usd = CASE" +
				"    WHEN q.monthly_window_start IS NULL OR v.monthly_window_start > q.monthly_window_start THEN v.monthly_usage_usd" +
				"    WHEN v.monthly_window_start = q.monthly_window_start THEN GREATEST(q.monthly_usage_usd, v.monthly_usage_usd)" +
				"    ELSE q.monthly_usage_usd END," +
				"  daily_window_start = GREATEST(q.daily_window_start, v.daily_window_start)," +
				"  weekly_window_start = GREATEST(q.weekly_window_start, v.weekly_window_start)," +
				"  monthly_window_start = GREATEST(q.monthly_window_start, v.monthly_window_start)," +
				"  updated_at           = $1" +
				" FROM (VALUES ")
