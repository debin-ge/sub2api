...
WHERE action = 'registration_reward'
  AND source_user_id = $1`, invitee.ID)
	...
	require.InDelta(t, 10, rechargeAccrued, 1e-9,
		"registration rewards must consume the shared per-invitee rebate cap")
...
func TestAffiliateRepository_BindInviterAndGrantRegistrationReward_RespectsPerInviteeCap(t *testing.T) {
	ctx := context.Background()
	tx := testEntTx(t)
	txCtx := dbent.NewTxContext(ctx, tx)
	client := tx.Client()
	...
	invitee := mustCreateUser(t, client, &service.User{
		Email:        fmt.Sprintf("registration-reward-cap-invitee-%d@example.com", time.Now().UnixNano()+1),
		PasswordHash: "hash",
		Role:         service.RoleUser,
		Status:       service.StatusActive,
	})
	...
	// Simulate cap-eligible rebate already generated by the same invitee. The
	// registration reward may only consume the remaining amount.
	applied, err := repo.AccrueQuota(txCtx, inviter.ID, invitee.ID, 4, 0, 0, nil)
	require.NoError(t, err)
	require.InDelta(t, 4, applied, 1e-9)
	...
	require.InDelta(t, 2.5, result.RewardAmount, 1e-9)

	require.InDelta(t, 6.5, querySingleFloat(t, txCtx, client,
...
FROM user_affiliate_ledger
WHERE action = 'registration_reward'
  AND source_user_id = $1`, invitee.ID), 1e-9)

	capEligibleTotal, err := repo.GetAccruedRebateFromInvitee(txCtx, inviter.ID, invitee.ID)
	require.NoError(t, err)
	require.InDelta(t, 6.5, capEligibleTotal, 1e-9)
}

func TestAffiliateRepository_AccrueQuota_ConcurrentRequestsRespectSharedCap(t *testing.T) {
	ctx := context.Background()
	...
	invitee := mustCreateUser(t, integrationEntClient, &service.User{
		Email:        fmt.Sprintf("concurrent-cap-invitee-%d@example.com", time.Now().UnixNano()+1),
		PasswordHash: "hash",
		Role:         service.RoleUser,
		Status:       service.StatusActive,
	})
...
WHERE action = 'accrue'
  AND source_user_id = $1`, invitee.ID), 1e-9)
	...
	capEligibleTotal, err := repo.GetAccruedRebateFromInvitee(ctx, inviter.ID, invitee.ID)
	require.NoError(t, err)
...
func TestAffiliateRepository_ListAffiliateInviteRecords_UsesInvitationTimeRunningTotal(t *testing.T) {
	...
	accrued, err := repo.AccrueQuota(txCtx, inviter.ID, firstInvitee.ID, 5, 0, 0, nil)
	require.NoError(t, err)
	require.InDelta(t, 5, accrued, 1e-9)
