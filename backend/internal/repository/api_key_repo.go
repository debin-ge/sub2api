...
func userEntityToService(u *dbent.User) *service.User {
	...
	out := &service.User{
		...
		VIPEntitlementSnapshot: service.VIPEntitlementSnapshot{
			PaidEligible:    u.VipPaidEligible,
			PaidEligibleAt:  u.VipPaidEligibleAt,
			PaidSource:      service.VIPPaidSource(u.VipPaidSource),
			ManualOverride:  u.VipManualOverride,
			OverrideAt:      u.VipOverrideAt,
			OverrideBy:      u.VipOverrideBy,
			OverrideReason:  u.VipOverrideReason,
			IsVIP:           u.IsVip,
			GrantedAt:       u.VipGrantedAt,
			EffectiveSource: service.VIPEffectiveSource(u.VipEffectiveSource),
		},
		RPMLimit:  u.RpmLimit,
