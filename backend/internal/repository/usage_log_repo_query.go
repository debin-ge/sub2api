...
func shouldUseFastUsageLogTotal(filters UsageLogFilters) bool {
	...
	// 结算状态筛选同样是强选择：待结算记录是全表里的异常少数，走部分索引后精确
	// COUNT(*) 本来就很便宜。而且这个视图的用途就是"还欠多少笔账"，给个估算总数
	// 等于没答。
	if filters.BillingState != nil || filters.BillingStateUnsettled {
		return false
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		billingState              int16
		usageSource               int16
		usageEstimationMethod     sql.NullString
	)
	...
	log := &service.UsageLog{
		...
		BillingState:              int8(billingState),
		UsageSource:               service.UsageSource(usageSource),
		UsageEstimationMethod:     usageEstimationMethod.String,
