-- Consolidate every groups column materialized in APIKeyAuthGroupSnapshot.
--
-- Two migrations with the 193 prefix both replaced this trigger function. The
-- profit-control migration sorts after the VIP/auth-snapshot extension and
-- therefore unintentionally narrowed the watched columns. Keep historical
-- migrations immutable and restore the union here so both upstream features
-- and the local VIP authorization fields converge across cached instances.
