...
func TestCompositeRoutesLocalProvidersMigration(t *testing.T) {
	content, err := FS.ReadFile("233_composite_routes_add_local_providers.sql")
	require.NoError(t, err)

	sql := strings.Join(strings.Fields(string(content)), " ")
	require.Contains(t, sql, "DROP CONSTRAINT IF EXISTS composite_model_routes_target_platform_check")
	require.Contains(t, sql,
		"CHECK (target_platform IN ( 'anthropic', 'openai', 'gemini', 'antigravity', 'grok', 'kimi', 'zhipu', 'deepseek', 'minimax', 'windsurf', 'opencode' ))")
	require.NotContains(t, sql, "'glm'")
}
